...
func TestCLI(t *testing.T) {
	for _, tc := range []struct {
		...
		{"version", []string{"--version"}, 0, "dots 0.0.0-dev", false, false},
		{"doctor", []string{"doctor"}, 0, "Platform: termux", false, true},
		{"doctor help", []string{"doctor", "--help"}, 0, "Usage: dots doctor", true, false},
...
func TestMetadata(t *testing.T) {
	var out, errOut bytes.Buffer
	r, err := registry(&out, &errOut, func() string { return "" }, nil)
	if err != nil {
		t.Fatal(err)
	}
	if len(r.Project()) != 5 {
	...
	for _, m := range r.Project() {
		if !strings.Contains(out.String(), m.Synopsis) || !strings.Contains(out.String(), m.Summary) {
			t.Fatal("help differs from registry projection")
		}
		for _, example := range m.Examples {
			var stdout, stderr bytes.Buffer
			args := strings.Fields(example)[1:]
			if Run(args, &stdout, &stderr, func() string { return "" }, func() platform.Report { return platform.Report{} }) != 0 {
