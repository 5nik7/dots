...
External acceptance additionally builds `tests/fixtures/command` into an owned `extension-fixture` executable and supplies `DOTS_EXTENSION_TEST_BIN`. Tests exercise strict mandatory JSON, limits, protected built-in precedence, duplicate roots/routes, unavailable/unsupported candidates, longest-prefix fallback refusal, empty-PATH execution, exact arguments (including empty/quotes/backslashes/metacharacters/Unicode), environment/cwd, binary stdin, separate streams, exit codes, and no-execution static help/discovery. Unix verifies process replacement and SIGINT/SIGTERM. Windows creates a disposable real console and broadcasts native Ctrl+C and Ctrl+Break to helper/wrapper/fixture; the wrapper must await the fixture's handled exit. Both console cases are required, never replaced by a skip or mocked cancellation. Test files and sentinel inputs are disposable; no actual management extension is provided.

The same warm batches now measure `external-dispatch` (one native no-op fixture process) and `discovery-10` (one explicit root, ten definitions) separately from help/version. Raw metadata records fixture and actual logo hashes. Deterministic resolver tests verify no enumeration and unchanged targeted operation counts with 1,000 unrelated entries; this is not a filesystem performance claim. No persistent cache is added. Existing help/version numeric budgets remain advisory; the new external/discovery numbers establish baselines, not newly invented enforced thresholds. Structural isolation/no-execution/bounded lookup and native process contracts are enforced.

Catalog acceptance covers exact schema keys/types, non-null arrays, global-only version, self-advertised JSON output, defensive sorting, typed availability reasons and stable bytes across root/insertion order. Native process tests check hidden/foreign-platform/missing-native/unsupported records, no implicit roots, empty stdout on malformed hidden metadata, duplicate roots/routes, invalid entries and discovery limits. Poison fixture output and unchanged-root snapshots prove static requests do not execute extensions or mutate their fixtures. Existing prefix precedence, targeted lookup, native interruption/cleanup and Python benchmark-count regressions remain required.

`bench` additionally labels `discovery-json-10` separately using the same ten definitions, three alternating batches, twenty warmups and two hundred samples per batch (600 total). Preserve the existing `discovery-10` text series, help/version, external execution and registry series. JSON timing is a new advisory baseline, not a controlled cold-start result or a new hard budget.
