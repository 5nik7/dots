...
// These error codes report an unavailable Windows capability, not test success.
func linkAvailable(t *testing.T, err error) {
	t.Helper()
	if runtime.GOOS == "windows" && (errors.Is(err, syscall.Errno(1314)) || errors.Is(err, syscall.Errno(5)) || errors.Is(err, syscall.Errno(50))) {
		t.Skipf("UNAVAILABLE: symlink permission/capability: %v; security policy unchanged", err)
	}
}

func link(t *testing.T, root *os.Root, from, to string) {
	t.Helper()
	err := root.Symlink(from, to)
	linkAvailable(t, err)
	if err != nil {
		t.Fatal(err)
	}
...
// Copy must execute even when no symlink privilege is available.
...
func TestExistingTargets(t *testing.T) {
	for _, strategy := range []string{"copy", "link"} {
		t.Run(strategy, func(t *testing.T) {
			for _, kind := range []string{"file", "directory", "broken link"} {
				t.Run(kind, func(t *testing.T) {
					...
					if strategy == "copy" {
						err = copyExclusive(r, "source", "target")
					} else {
...
func TestMissingCopyParent(t *testing.T) {
	r := fixture(t)
	write(t, r, "source", "contents")
	if err := copyExclusive(r, "source", "missing/copy"); !errors.Is(err, os.ErrNotExist) {
...
func TestRootEscapeRefused(t *testing.T) {
	for _, kind := range []string{"traversal", "symlink"} {
		t.Run(kind, func(t *testing.T) {
			// A failing assertion still targets only owned escape sentinels.
			outer := fixture(t)
			...
			if kind == "traversal" {
				...
				if runtime.GOOS == "windows" {
					if err := copyExclusive(r, "source", `..\escape`); err == nil {
						t.Fatal("Windows separator traversal accepted")
					}
				}
			} else {
				link(t, r, "../sentinel", "outside")
