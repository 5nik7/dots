...
The working direction is a small compiled core, currently proposed in Go, with POSIX shell and PowerShell bootstrap scripts and an Omarchy-inspired external command protocol. The implementation language is not final until a Termux portability spike proves startup, filesystem, symlink, and release behavior.
