...
function dotenv {
    ...
    if (Test-Path $envFilePath) {
        if ($v) {
            wh 'DOTENV' white ' │ ' darkgray 'LOADING' darkgray ' │ ' darkgray "$env:DOTS\" blue '.env' green -box -border 0 -bb 1 -ba 1 -pad $env:padding
        }
...
if ($env:isReloading) {
    Clear-Host
    wh 'Profile reloaded.' green -box -border darkgray -bb 1 -ba 1 -pad $env:padding
