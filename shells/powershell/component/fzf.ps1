...
$env:FZF_DEFAULT_COMMAND = $FZF_DEFAULT_COMMAND
 ...
 $fzf_opts = @{
        ...
        $_FZF_DEFAULT_OPTS = ($fzf_opts.GetEnumerator() | foreach-object { "--{0}=""{1}""" -f $_.Key, $_.Value }) -join ' '
         ...
         $env:FZF_DEFAULT_OPTS = "--style=full --ansi --height=65% --multi --ansi --padding=0 --margin=0 --color=bg+:0,bg:-1,spinner:4 --color=fg:8,header:3,info:8,pointer:13 --color=hl:7:underline,hl+:5:reverse --color=marker:14,fg+:13,prompt:8 --color=gutter:-1,selected-bg:0,separator:$bg --color=preview-bg:$bg,preview-label:0,label:7,query:5 --color=border:0,list-border:0,preview-border:0,input-border:0 --info=inline --layout=reverse --prompt=' ' --pointer='┃' --separator='──' --marker='│' --scrollbar='│' --border=sharp  --preview-window=right:50%,border-sharp --preview='bat --style=numbers --color=always {}'"
