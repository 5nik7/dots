...
def sanitized(text):
    ...
    for spelling in (temporary, temporary.replace("\\", "/")):
        text = re.sub(re.escape(spelling) + r"[/\\]dots-core-work-[^/\\\s'\"<>]+", "<work>", text)
    return text
...
def collect(temporary, runner_os, env):
    ...
    output = temporary / ("dots-core-" + runner_os.lower() + "-evidence")
    ...
    runner["limitations"] = runner_os + " native CI execution; fresh warm startup and in-process registry measurements; no controlled cold-cache or representative desktop hardware baseline"
    write_json(output / "runner.json", runner)
    print(json.dumps(runner, indent=2), flush=True)
    results = []
    hashes = []
    for mode in ("check", "bench"):
        command = [sys.executable, "-B", str(verify.MODULE / "tools" / "verify_core.py"), mode]
        ...
        results.append({"mode": mode,
                        "command": ("python" if runner_os == "Windows" else "python3") + " -B tools/verify_core.py " + mode,
        ...
        if artifact.parent != temporary or not artifact.name.startswith("dots-core-artifact-") or binary.name != verify.executable_name():
        ...
        # Include sanitized raw timing batches for durable review through job logs
        # as well as the artifact when artifact-download transport is unavailable.
        print(json.dumps({key: value for key, value in metadata.items() if key not in ("elf", "dependencies")}, indent=2), flush=True)
        if mode == "bench":
            print(json.dumps({"startup": metadata["benchmark"]["summary"], "registry": metadata["registry_benchmark"]}, indent=2), flush=True)
    ...
    print("Native " + runner_os + " core validation and evidence collection passed.", flush=True)
